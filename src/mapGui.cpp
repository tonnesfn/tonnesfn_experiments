...
public:
  ...
  void eval(Indiv& ind) {
    ...
    // Size always one in map experiments
    //this->_objs.resize(0);
    ...
    data.push_back((individualParameters[7] + individualParameters[8]) / 120.0f); // Total leg height
...
void experiments_evolve(const std::string givenMorphology, bool evolveMorphology, bool givenAddDiversity){
  ...
  for (int i = 0; i < numberOfTests; i++){
    ...
    float best = expGui::ea.stat<1>().best()->fit().value();

    fprintf(logOutput, "best fit (map_elites): %f\n", best);

    evoLogPath.clear();

    fprintf(logOutput, "Experiment finished1. Log written to:\n  %s\n", ss.str().c_str());
  }
